...
# Get the stock_name argument
stock_name = sys.argv[1] if len(sys.argv) > 1 else None

# if stock_name:
#     st.title(f'Stock Prediction for {stock_name}')
#     # Perform stock prediction based on the stock_name
# else:
#     st.write('Please enter a stock name in the Django app form.')

st.title(f'Stock Trend Prediction of {stock_name}')
# user_input=st.text_input('Enter Stock Ticker','AAPL')
current_date = datetime.datetime.now().date()
df=yf.download(stock_name,start = '2016-01-01',end = current_date )
###current status###
